...
<script language="javascript" type="text/javascript">
	function companycode() {
		...
		url += "?count=" + s1; // + "&sec=" + s;
	//alert(url);
		xmlHttp002.onreadystatechange = codeAdd04;
...
<s:actionmessage/>
  ...
  <legend style="color:red;"><h3>Alteration of Item group</h3></legend>
  
  <s:form action="materialgroupcreation1" namespace="/" method="post" autocomplete="off">
  <input type="hidden" name="formbean.id" value="<s:property value="formbean.id"/>"/>
  <input type="hidden" name="formbean.groupcode" value="<s:property value="formbean.groupcode"/>"/>
  ...
  <table cellpadding="5" width="70%" style="padding:20px">  
  		...
  		<tr>	
      		<td width="12.5%" style="padding-left:15px; padding-right:15px;"><label>Chose&nbsp;Group&nbsp;Description</label></td>
        	<td width="12.5%" style="padding-left:15px; padding-right:15px;"><s:textfield name="formbean.desc"  id="desc" size="30" cssStyle="height:25px;border:1px solid #ccc;background:#FFFF99"></s:textfield></td>
       </tr>
       <tr>
         ...
         <tr>
        	...
        	<td width="12.5%" style="padding-left:15px; padding-right:15px;"><sj:autocompleter name="formbean.companyname" id="companyname" list="usList3"  listValue="companyname" size="50" cssStyle="height:25px;border:1px solid #ccc;background:#FFFF99" forceValidOption="false" onfocusout="companycode();"></sj:autocompleter></td>
        </tr>
	...
	<s:if test="usList">
	<div id="ram">
	<display:table id="table12"  name="usList" pagesize="25"  class="imagetable" requestURI=""     export="true" style="width:60%;color:#000">
	    
	 <s:if test="%{#attr.table12.id == formbean.id}">
	
	<display:column property="desc"   title="Description Name" style="background-color:#ecf2f6;"   sortable="false" headerClass="sortable" />
    ...
    <a href="<s:url action="materialupdate1" namespace="/" ><s:param  name="id" value="%{#attr.table12.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="classname">Update</a>
    </display:column>
    
    <display:column title="Delete" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="<s:url action="materialdelete1" namespace="/" ><s:param  name="id" value="%{#attr.table12.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="print" onclick="return confirmation();">Delete</a>
    </display:column>
	</s:if>
