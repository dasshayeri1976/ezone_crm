...
function confirmation()
{
	...
	if (r == true) 
	{
		var m=document.getElementById('citycode').value;
		//alert('m: '+m);
		if(m==1)
		{
			alert("city is Already Tag With place data, So Operation Cannot be proceed");
			x= false;
		}
		else
			x=true;
...
<script language="javascript" type="text/javascript">
	...
	function cityrecordcheck(a) {
		...
		var url = "./Body/deletemaster/cityrecordcheck.jsp";
		url += "?citycode=" + a; // + "&sec=" + s;
	...
	function codeAdd1() {
		if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
			...
			document.getElementById('citycode').value=x1;
			//avi(x);
			//alert(x1);
			//comfirmation(x);
...
<script language="javascript" type="text/javascript">
	...
	function statecode() {
		...
		var url = "./Body/master/statename.jsp";
		url += "?count=" + s1; // + "&sec=" + s;
		//url +="?count=" +str+"&date1="+s;
	...
	function codeAdd() {
		if (xmlHttp1.readyState == 4 || xmlHttp1.readyState == "complete") {
			...
			document.getElementById("state").value = x[1];
...
<s:actionmessage/>
  ...
  <legend style="color:red;">City Creation</legend>
  
  <s:form action="citycreation" namespace="/" method="post">
  ...
  <table width="50%" style="padding:10px;" >
	 ...
	 <tr>
        ...
        <td width="12.5%" style="padding-left:15px; padding-right:15px;"><s:select name="formbean.sname" list="usList1" id="sname" listKey="sname" value="formbean.sname" listValue="sname" cssStyle="height:20px;border:1px solid #ccc;background:#FFFF99;width:100px;" onchange="statecode();" tabindex="3"/></td>
        <td width="12.5%" style="padding-left:15px; padding-right:15px;">State_Name</td>
        <td width="12.5%" style="padding-left:15px; padding-right:15px;"><s:textfield name="formbean.state" id="state"  size="30" cssStyle="height:20px;border:1px solid #ccc;background:#FFFF99" readonly="true" tabindex="4"></s:textfield></td>
    ...
    <br/>
      ...
      <div align="right" style="width:99%;position:relative;padding:7px;">
      	...
      	<s:submit name="formbean.save" value="Save City" cssClass="butStnd" onclick="return Validate()" ></s:submit>
	...
	<display:column property="name"   title="City Name" style="background-color:#ecf2f6;"   sortable="false" headerClass="sortable" />
	<display:column property="sname"   title="District Name" style="background-color:#ecf2f6;"   sortable="false" headerClass="sortable" />
	<display:column property="state"   title="State Name" style="background-color:#ecf2f6;"   sortable="false" headerClass="sortable" />
	
	<display:column title="Update" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="<s:url action="cityupdate" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="classname">Update</a>
    </display:column>
    
    <display:column title="Delete" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="<s:url action="citydelete" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="print" onclick="return confirmation();">Delete</a>
    </display:column>
	</s:if>
	<s:else>
		 
    <display:column property="name"   title="City Name"  style="background-color:#fff;"  sortable="false" headerClass="sortable" />
	<display:column property="sname"   title="District Name"  style="background-color:#fff;"  sortable="false" headerClass="sortable" />
	<display:column property="state"   title="State Name"  style="background-color:#fff;"  sortable="false" headerClass="sortable" />-->
    ...
    <a href="<s:url action="citydelete" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color: #fff;text-decoration:none;font-size:12px;" class="print" onclick="cityrecordcheck('<s:property value="%{#attr.table.scode}"/>');return confirmation();">Delete</a>
    </display:column>
