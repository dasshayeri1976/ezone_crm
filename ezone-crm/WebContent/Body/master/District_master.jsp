...
function confirmation()
{
	...
	if (r == true) 
	{
		var m=document.getElementById('districtcode').value;
		//alert('m: '+m);
		if(m==1)
		{
			alert("District is Already Tag With City data, So Operation Cannot be proceed");
			x= false;
		}
		else
			x=true;
...
<script language="javascript" type="text/javascript">
	...
	var xmlHttp;
	var x1='';
	function districtrecordcheck(a) {
		//alert(a);
		...
		var url = "./Body/deletemaster/districtrecordcheck.jsp";
		url += "?districtcode=" + a; // + "&sec=" + s;
	...
	function codeAdd1() {
		if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
			...
			document.getElementById('districtcode').value=x1;
			//avi(x);
			//alert(x1);
			//comfirmation(x);
...
<script language="javascript" type="text/javascript">
	...
	function statecode() {
		...
		var url = "./Body/master/statecode.jsp";
		url += "?count=" + s1; // + "&sec=" + s;
		//url +="?count=" +str+"&date1="+s;
...
<s:actionmessage/>
  ...
  <legend style="color:red;"><h3>District Creation</h3></legend>
  
  <s:form action="districtcreation" namespace="/" method="post">
  ...
  <table width="50%" style="padding:10px;" >
	 ...
	 <tr>   
        ...
        <td width="12.5%" style="padding-left:15px; padding-right:15px;"><s:select name="formbean.sname" list="usList1" id="sname" listKey="sname" value="formbean.sname" listValue="sname" cssStyle="height:20px;border:1px solid #ccc;background:#FFFF99;width:100px;" onchange="statecode();" tabindex="2"/></td>
       </tr>
	...
	<display:column property="name"   title="District Name" style="background-color:#ecf2f6;"   sortable="false" headerClass="sortable" />	
	<display:column property="sname"   title="State Name" style="background-color:#ecf2f6;"   sortable="false" headerClass="sortable" />
	
	<display:column title="Update" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="<s:url action="districtupdate" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="classname">Update</a>
    </display:column>
    
    <display:column title="Delete" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="<s:url action="districtdelete" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="print" onclick="return confirmation();">Delete</a>
    </display:column>
	</s:if>
	<s:else>
		 
	<display:column property="name"   title="District Name"  style="background-color:#fff;"  sortable="false" headerClass="sortable" />
	<display:column property="sname"   title="State Name"  style="background-color:#fff;"  sortable="false" headerClass="sortable" />
    ...
    <a href="<s:url action="districtdelete" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color: #fff;text-decoration:none;font-size:12px;" class="print" onclick="districtrecordcheck('<s:property value="%{#attr.table.scode}"/>');return confirmation();">Delete</a>
    </display:column>
